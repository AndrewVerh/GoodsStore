...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < products.length; i++) {
            sumSale += prices[i] * counts[i];
        }
        boolean doBonus = sumSale >= MIN_COST_FOR_BONUS;
        for (int i = 0; i < products.length; i++) {
            if (counts[i] != 0) {
                ...
                if (isOnSale && counts[i] >= 3) { //распродажа если акционных товаров >=3 и +1 товар если корзина >= 1000
                ...
                } else if (sumSale >= MIN_COST_FOR_BONUS) { //акция если сумма корзины >=1000
